...
def log(msg: str):
    """Append a timestamped line to scheduler.log and print to stdout."""
...
def append_rows(rows: list[dict]):
    """Append a batch of fetched rows to live_feed.csv."""
    if not rows:
        return
...
def run():
    ...
    while True:
        ...
        if market_status() == "after" or day_closed:
            log("Market closed. Scheduler done for today.")
            break
